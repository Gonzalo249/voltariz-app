...
const Navbar = ({ activeView, setActiveView }: { activeView: string, setActiveView: (view: string) => void }) => {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 transition-all duration-500 bg-[#02105b] backdrop-blur-2xl border-b border-white/5 py-4 shadow-lg overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
        <div className="md:col-span-1">
          ...
          <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest leading-loose">
            © 2024 Voltariz Energy. Precision engineering for a sustainable future.
          </p>
        </div>
        ...
        <div className="md:col-span-1">
          ...
          <div className="space-y-4 text-xs font-medium text-white/60">
            <p className="flex items-start gap-3">
              <MapPin size={16} className="text-[#131d26] shrink-0 mt-0.5" />
              <a href="https://maps.google.com/?q=Paseo+de+la+Castellana+259,Madrid,ES" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">
                Paseo de la Castellana 259<br/>Madrid, España
              </a>
            </p>
            <p className="flex items-center gap-3">
              <Phone size={16} className="text-[#131d26] shrink-0" />
              <a href="tel:+34912345678" className="hover:text-white transition-colors">+34 912 345 678</a>
            </p>
            <p className="flex items-center gap-3">
              <MessageCircle size={16} className="text-[#131d26] shrink-0" />
              <a href="https://wa.me/34912345678" target="_blank" rel="noreferrer" className="hover:text-white transition-colors">WhatsApp</a>
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-1">
          ...
          <div className="flex space-x-4">
            <motion.a whileHover={{ scale: 1.1, backgroundColor: '#131d26', color: '#ffffff' }} href="#" className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white/50 transition-all cursor-pointer">
              <Instagram size={16} />
            </motion.a>
            <motion.a whileHover={{ scale: 1.1, backgroundColor: '#131d26', color: '#ffffff' }} href="#" className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white/50 transition-all cursor-pointer">
              <Facebook size={16} />
        ...
        <div className="md:col-span-1 flex flex-col space-y-4 text-[10px] font-bold uppercase tracking-widest text-white/60">
          ...
          <motion.a whileHover={{ x: 5, color: '#ffffff' }} href="#" className="hover:text-white transition-colors cursor-pointer w-fit">Privacidad</motion.a>
          <motion.a whileHover={{ x: 5, color: '#ffffff' }} href="#" className="hover:text-white transition-colors cursor-pointer w-fit">Términos</motion.a>
          <motion.button 
            whileHover={{ x: 5, color: '#ffffff' }}
            onClick={() => {
              const event = new CustomEvent('navToAdmin');
              window.dispatchEvent(event);
            }} 
            className="hover:text-white transition-colors opacity-50 cursor-pointer w-fit text-left"
          >
            Sistemas
          </motion.button>
        </div>
...
const AdminView = () => {
  const [stats] = useState([
    { label: 'Leads Totales', value: '1,284', icon: <Mail size={20} />, trend: '+12%' },
    { label: 'Proyectos Activos', value: '42', icon: <Briefcase size={20} />, trend: '+5%' },
    { label: 'Ahorro Generado', value: '$2.4M', icon: <Zap size={20} />, trend: '+18%' },
    { label: 'Satisfacción', value: '99.2%', icon: <ShieldCheck size={20} />, trend: '0%' }
  ]);

  const [leads] = useState([
    { id: 1, name: 'Carlos Mendoza', email: 'carlos@industrial.com', type: 'Industrial', date: '2024-03-28', status: 'Nuevo' },
    { id: 2, name: 'Elena Rivas', email: 'elena@residence.com', type: 'Residencial', date: '2024-03-27', status: 'En Proceso' },
    { id: 3, name: 'Roberto Solis', email: 'r.solis@corp.mx', type: 'Corporativo', date: '2024-03-26', status: 'Atendido' },
    { id: 4, name: 'Arquitectura G&A', email: 'vargas@gya.es', type: 'High-End', date: '2024-03-25', status: 'Nuevo' }
  ]);
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50">
                  ...
                  <th className="px-10 py-6 text-[10px] font-bold text-navy/30 uppercase tracking-widest">Estado</th>
              ...
              <tbody className="divide-y divide-navy/5">
                {leads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-10 py-6">
                      <span className="text-xs font-bold text-navy/60">{lead.type}</span>
                    </td>
                    <td className="px-10 py-6">
                      <span className="text-xs font-medium text-navy/40">{lead.date}</span>
                    </td>
                    <td className="px-10 py-6">
                      <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${
                        lead.status === 'Nuevo' ? 'bg-[#131d26]/10 text-[#131d26]' : 
                        lead.status === 'Atendido' ? 'bg-green-100 text-green-600' : 
                    ...
                    <td className="px-10 py-6 text-right">
                      <motion.button 
                        whileHover={{ scale: 1.2, color: '#03245d' }}
                        whileTap={{ scale: 0.9 }}
                        className="p-2 opacity-0 group-hover:opacity-100 transition-opacity text-navy/20 hover:text-navy cursor-pointer"
                      >
          ...
          >
            {[
              ...
              >
                  ...
                  {item.image && (
                    <div className="absolute inset-0 z-0 opacity-15 group-hover:opacity-100 transition-opacity duration-300">
                      <img 
                        src={item.image} 
                        alt={item.title} 
                        className="w-full h-full object-cover" 
                  ...
                  <div className="flex flex-col items-center text-center relative z-10">
                    ...
                    <div className="flex items-center text-[10px] font-bold text-[#131d26] tracking-widest uppercase transition-all group-hover:text-white">
    ...
    <div className="pt-32 relative overflow-hidden bg-white">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 mb-32 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-6">
            <div className="aspect-[4/5] bg-charcoal/5 rounded-2xl overflow-hidden shadow-2xl">
              <img 
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 mb-32">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-12 lg:col-span-7 bg-navy border border-white/5 p-12 flex flex-col justify-between">
            ...
            <div className="aspect-video bg-white/5 rounded-xl overflow-hidden mt-auto">
               <img src="/images/paneles-solare-casa-1024x768.webp" className="w-full h-full object-cover grayscale brightness-110" />
            </div>
          </div>
          ...
          <div className="md:col-span-6 lg:col-span-5 bg-charcoal p-12 text-white flex flex-col justify-between overflow-hidden relative group">
            <div className="absolute inset-0 opacity-10 group-hover:opacity-20 transition-opacity">
              <img src="/images/panel-solar-comercial.webp" className="w-full h-full object-cover" />
            </div>
          ...
          <div className="md:col-span-6 lg:col-span-5 bg-charcoal p-12 flex flex-col">
            ...
            <div className="aspect-square bg-white/5 shadow-xl rounded-xl p-4 mt-auto">
               <img src="/images/panel-solar-industrial.webp" className="w-full h-full object-cover rounded-lg grayscale brightness-110" />
            </div>
          </div>
    ...
    <div className="pt-32 relative overflow-hidden bg-white">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 mb-32 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 h-[500px] bg-charcoal/5 rounded-2xl overflow-hidden relative grayscale shadow-2xl">
            <img 
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 mb-32 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-6 bg-charcoal/5 border border-charcoal/5 p-16 relative overflow-hidden group">
          <span className="absolute top-0 right-0 text-[180px] font-black text-charcoal/5 transform translate-x-12 -translate-y-12">01</span>
          <div className="relative z-10">
        ...
        <div className="lg:col-span-6 bg-charcoal/5 border border-charcoal/5 p-16 relative overflow-hidden group">
          <span className="absolute top-0 right-0 text-[180px] font-black text-charcoal/5 transform translate-x-12 -translate-y-12">02</span>
          <div className="relative z-10">
        ...
        {/* Engineering Highlight */}
            <div className="lg:col-span-12 bg-white text-charcoal p-12 md:p-24 relative overflow-hidden group rounded-2xl border-l-8 border-[#131d26]">
              <div className="absolute top-0 left-0 text-[280px] font-black text-charcoal/5 transform -translate-x-20 -translate-y-32">03</div>
          <div className="lg:grid lg:grid-cols-2 gap-20 items-center relative z-10">
            <div>
              <Zap className="text-charcoal/40 mb-12" size={48} />
              <h3 className="text-5xl md:text-7xl font-medium mb-10">Ingeniería</h3>
              <p className="text-xl text-charcoal/70 leading-relaxed mb-12 max-w-xl">
                 Nuestra fase más crítica. Diseñamos planos estructurales, diagramas unifilares y cálculos de carga con precisión milimétrica, garantizando la longevidad del sistema.
              </p>
              <div className="flex flex-wrap gap-4 underline decoration-charcoal/20 underline-offset-8 text-xs font-bold tracking-widest uppercase">
                <span>Planos CAD</span>
                <span>Cálculo Estructural</span>
                <span>Análisis de Red</span>
              </div>
            </div>
            <div className="mt-16 lg:mt-0 rounded-xl overflow-hidden aspect-video border border-charcoal/10">
               <img src="https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=2670&auto=format&fit=crop" className="w-full h-full object-cover opacity-80" />
            </div>
          </div>
        </div>

        <div className="lg:col-span-7 bg-charcoal/5 border border-charcoal/5 p-16 relative overflow-hidden group">
          <span className="absolute top-0 right-0 text-[180px] font-black text-charcoal/5 transform translate-x-12 -translate-y-12">04</span>
          <div className="relative z-10">
            <div className="p-3 bg-white shadow-sm inline-block rounded-lg mb-10"><Zap className="text-charcoal" size={32} /></div>
        ...
        <div className="lg:col-span-5 bg-charcoal/5 border border-charcoal/5 p-16 relative overflow-hidden group">
          <span className="absolute top-0 right-0 text-[180px] font-black text-charcoal/5 transform translate-x-12 -translate-y-12">05</span>
          <div className="relative z-10">
            <div className="p-3 bg-white shadow-sm inline-block rounded-lg mb-10"><BarChart3 className="text-charcoal" size={32} /></div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-12 mb-24">
          <div className="max-w-xl">
            <div className="relative pl-6 mb-8 border-l-4 border-[#131d26] text-charcoal">
              <h2 className="text-4xl md:text-6xl font-medium">Excelencia Técnica en Cada Conexión</h2>
            </div>
...
const ContactView = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    phone: '',
    projectType: 'Residencial High-End',
    message: ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setSubmitted(true);
      setFormData({ name: '', email: '', phone: '', projectType: 'Residencial High-End', message: '' });
    }, 1000);
  };
        ...
        <div className="lg:col-span-5 flex flex-col justify-center">
          ...
          <div className="mt-20 pt-16 border-t border-[#131d26]/10">
            <div className="aspect-video rounded-2xl overflow-hidden shadow-2xl relative group">
               <img src="https://images.unsplash.com/photo-1542332213-31f87348057f?q=80&w=2670&auto=format&fit=crop" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 grayscale brightness-110" />
            </div>
          </div>
        </div>
          ...
          <div className="max-w-xl mx-auto relative z-10">
            {submitted ? (
              ...
              >
                ...
                <p className="text-white/40 text-lg mb-12">Un ingeniero consultor se pondrá en contacto con usted en menos de 24 horas.</p>
                <motion.button 
                  whileHover={{ scale: 1.05, filter: 'brightness(1.1)' }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => setSubmitted(false)}
                  className="bg-white text-navy px-10 py-4 text-xs font-bold tracking-[0.2em] hover:bg-gray-100 transition-all cursor-pointer"
                  style={{
                    clipPath: "polygon(12px 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%, 0 12px)"
                  }}
                >
                  <span className="block">VOLVER A ENVIAR</span>
                </motion.button>
              </motion.div>
            ) : (
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="flex flex-col space-y-3">
                      ...
                      >
                        <option className="bg-navy text-white">Residencial High-End</option>
                        <option className="bg-navy text-white">Corporativo / Industrial</option>
                        <option className="bg-navy text-white">Infraestructura Pública</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="pt-10">
                    <motion.button 
                      ...
                      whileTap={{ scale: 0.98 }}
                      type="submit" 
                      className="group w-full md:w-auto bg-white text-navy px-16 py-6 font-bold text-xs tracking-[0.2em] hover:bg-gray-100 transition-all flex items-center justify-center gap-4 shadow-2xl shadow-black/20 cursor-pointer border-r-4 border-[#131d26]"
                      style={{
                        clipPath: "polygon(12px 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%, 0 12px)"
                      }}
                    >
            ...
            <div className="mt-24 pt-12 border-t border-white/5 grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <p className="text-white font-bold text-sm">Madrid, ES — Paseo de la Castellana 259</p>
              </div>
              <div>
                <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest mb-3">Consultas Directas</p>
                <p className="text-white font-bold text-sm">+34 912 345 678</p>
              </div>
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('navToAdmin', handleNavToAdmin);
  }, []);
